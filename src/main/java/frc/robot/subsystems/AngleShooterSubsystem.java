...
public class AngleShooterSubsystem extends SubsystemBase {
  ...
  public AngleShooterSubsystem() {
    ...
    runsValid = 0;
    // creates and applies configurations for the SparkMax Motor that controls the
    // shooter's angle
    ...
    // creates and applies configurations for the onboard Closed Loop (PID)
    // controller
    pitchPIDConfig = new ClosedLoopConfig();
    pitchPIDConfig.feedbackSensor(FeedbackSensor.kAbsoluteEncoder);
    pitchPIDConfig.pid(ANGLE_SHOOTER_POWER_KP, ANGLE_SHOOTER_POWER_KI, ANGLE_SHOOTER_POWER_KD);
    pitchPIDConfig.outputRange(pitchMinOutput, pitchMaxOutput);
  }

  /**
   * sets the setpoint for the pitch motor's onboard PID loop to be this angle. It should make the
   * whole indexer and shooter snap to that angle.
   *
   * <p>0 degrees is parallel with the ground
   *
  ...
  public void setDesiredShooterAngle(double degrees) {
    ...
    if (degrees > MaxAngle) {
      degrees = COMP_MAXIMUM_SHOOTER_ANGLE;
    }
    if (degrees < MINIMUM_SHOOTER_ANGLE) {
      degrees = MINIMUM_SHOOTER_ANGLE;
    }
    pitchPIDConfig.pidf(
    ...
    pitchMotor.getClosedLoopController().setReference(degrees, ControlType.kPosition);
  }

  /**
   * sets the pitch motor to a percent of it's full power.
   *
   * @param pitchSpeed Max: 1 Min: -1
   */
  ...
  /**
   * @return the current degrees of the shooter and indexer
   *     <p>0 degrees is parallel with the ground
   */
   ...
   *
   * @param pose the current Pose2d of the robot's location, from the odometry
   */
   ...
   *
   * @param speeds the current ChassisSpeeds of the drivetrain, gotten using
  ...
  /**
   * a math method that will use trigonometry and other math to calculate the current anlge that the
   * shooter should be at to point at the speaker
   *
   * <p>this method uses the variables inside the AngleShooterSubsytem: DTChassisSpeeds and
   * dtvalues, so make sure you set those values before running this method
   *
   * <p>you can set those values using {@code setDTPose(Pose2d)} and {@code
   * SetDTChassisSpeeds(ChassisSpeeds)}
   *
   * @return the desired angle, in degrees, for the shooter
   */
  public double calculateSpeakerAngle() {
    ...
    double totalDistToSpeaker =
        Math.sqrt(
            Math.pow(Field.SPEAKER_Z - ShooterConstants.SHOOTER_HEIGHT, 2)
                + Math.pow(speakerDist, 2));
    double shootingTime =
        totalDistToSpeaker
            / shootingSpeed; // calculates how long the note will take to reach the target
    ...
    Translation2d targetOffset =
        new Translation2d(
            currentXSpeed * shootingTime * OFFSET_MULTIPLIER * unadjustedAngle.getRadians(),
            currentYSpeed * shootingTime * OFFSET_MULTIPLIER * unadjustedAngle.getRadians());
    ...
    offsetSpeakerdist =
        offsetSpeakerdist
            + 0.127; // to compensate for the pivot point of the shooter bieng offset from the
    // center of the robot
    SmartDashboard.putString("offset amount", targetOffset.toString());
    SmartDashboard.putString(
        "offset speaker location", new Translation2d(offsetSpeakerX, offsetSpeakerY).toString());
    // getting desired robot angle
    double totalOffsetDistToSpeaker =
        Math.sqrt(
            Math.pow(offsetSpeakerdist, 2)
                + Math.pow(Field.SPEAKER_Z - ShooterConstants.SHOOTER_HEIGHT, 2));
    SmartDashboard.putNumber("MATH/shooter's speaker dist", totalOffsetDistToSpeaker);
    double desiredShooterAngle =
        Math.toDegrees(
            Math.asin(
                (Field.SPEAKER_Z - ShooterConstants.SHOOTER_HEIGHT) / totalOffsetDistToSpeaker));
  ...
  public double calculateSpeakerAngleWithSuppliers(
      DoubleSupplier redShooterXSup,
      DoubleSupplier blueShooterXSup,
      DoubleSupplier redYSup,
      DoubleSupplier blueYSup) {
    double deltaX;
    double deltaY;
  ...
  /**
   * using the constants made for this, it adjusts our shooter angle based on how far away we are
   * from the speaker
   *
   * @param initialAngle the angle that the {@code calculateSpeakerAngle()} method calculates before
   *     using this
  ...
  private double adjustAngleForDistance(double initialAngle, double distance) {
    ...
    if (Preferences.getBoolean("CompBot", false)) {
      double errorMeters =
          AdjustEquationA * Math.pow(distance, 3)
              + AdjustEquationB * Math.pow(distance, 2)
              + AdjustEquationC * distance
              + AdjustEquationD;
  ...
  /**
   * checks if the shooter has been at the correct angle for more than 20 (from constants) command
   * scheduler loops
   *
  ...
  public void periodic() {
    // SmartDashboard.putNumber("ANGLE SHOOTER shooter angle encoder position",
    // absoluteEncoder.getPosition()*360);
    SmartDashboard.putNumber("ANGLE SHOOTER absolute encoder value", absoluteEncoder.getPosition());
    // desiredZeroOffset = SmartDashboard.getNumber("ANGLE SHOOTER encoder zero
    // offset",
    // absoluteEncoder.getZeroOffset());
    SmartDashboard.putNumber("ANGLE SHOOTER encoder zero offset", zeroOffset);

    SmartDashboard.putNumber(
        "ANGLE SHOOTER speaker angle error", calculateSpeakerAngleDifference());

    SmartDashboard.putNumber(
        "CALLIBRATION/Supplied calculated shooter angle",
        calculateSpeakerAngleWithSuppliers(
            () -> SmartDashboard.getNumber("CALLIBRATION/redShooterX", 0),
            () -> SmartDashboard.getNumber("CALLIBRATION/blueShooterX", 0),
            () -> SmartDashboard.getNumber("CALLIBRATION/redY", 0),
            () -> SmartDashboard.getNumber("CALLIBRATION/blueY", 0)));
