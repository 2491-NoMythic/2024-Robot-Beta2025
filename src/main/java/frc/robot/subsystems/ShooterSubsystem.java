...
public class ShooterSubsystem extends SubsystemBase {
  ...
  public ShooterSubsystem(double runSpeed) {
    if (Preferences.getBoolean("CompBot", true)) {
      PIDRightconfigs =
          new Slot0Configs()
              .withKP(ShooterConstants.CompRightkP)
              .withKV(ShooterConstants.CompRightkFF)
              .withKI(0.004);
      PIDLeftconfigs =
          new Slot0Configs()
              .withKP(ShooterConstants.CompLeftkP)
              .withKV(ShooterConstants.CompLeftkFF)
              .withKI(0.004);
    } else {
      PIDRightconfigs =
          new Slot0Configs()
              .withKP(ShooterConstants.PracRightkP)
              .withKV(ShooterConstants.PracRightkFF)
              .withKI(0);
      PIDLeftconfigs =
          new Slot0Configs()
              .withKP(ShooterConstants.PracLeftkP)
              .withKV(ShooterConstants.PracLeftkFF)
              .withKI(0);
    }
    runsValid = 0;
  ...
  public void setTargetVelocity(
      double RPS_Left, double RPS_Right, double supplyLimit, double statorLimit) {
  ...
  // }
  /**
   * configures the motor's current limits to our desired limit's while shooting, and then sets the
   * setpoint of both shooter motor's onboard PID loop. The left motor will run at half the desired
   * speed, and the right motor will run at full. To run the motors at the same speed, use {@code
   * shooter.shootSameRPS(RPS)}
   *
  ...
  /**
   * allows you to set the shooter's speed using a supplier. this way you can use a value on
   * SmartDashboard to tune the shooter's speed.
   *
   * <p>if sameSpeed is true, than the wheels will run at the same RPS. Otherwise, the left wheel
  ...
  /**
   * configures the motor's current limits, then it set's the setpoint for the motor's onboard PID
   * loop based on the RPS the left motor will run at half the RPS, and the right motor will run at
   * te full RPS
   *
  ...
  /**
   * returns the absolute value of the right shooter motor's onboard PID loop. AKA it tells you how
   * far away the right shooter motor's speed is from what we've told it to be
   *
  ...
  /**
   * checks if the shooter has been within ALLOWED_SPEED_ERROR (from constants) for more than 20
   * (from constants) commandScheduler loops
   *
  ...
  /**
   * turns off both of the shooter motors. We use percentage-of-full-power so that the wheels will
   * coast to a stop and not use unnecessary power to stop them instantly.
   */
  public void turnOff() {
    targetVelocityL = 0;
    targetVelocityR = 0;
  ...
  private static void updateMotor(
      TalonFX shooterMotor, int revState, double targetSpeed, CurrentLimitsConfigs currentLimits) {
  ...
  /**
   * Revs left and right motors while below target velocity, if above target velocity, the motors
   * are put to PID Mode
   */
  private void updateMotors() {
    revStateL =
        updateIsReving(revStateL, targetVelocityL, shooterL.getVelocity().getValueAsDouble());
    revStateR =
        updateIsReving(revStateR, targetVelocityR, shooterR.getVelocity().getValueAsDouble());
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber(
        "shooter current right", shooterR.getSupplyCurrent().getValueAsDouble());
    SmartDashboard.putNumber(
        "shooter current left", shooterL.getSupplyCurrent().getValueAsDouble());
    SmartDashboard.putNumber(
        "Shooter/Right shooter speed", shooterR.getVelocity().getValueAsDouble());
    SmartDashboard.putNumber(
        "Shooter/Left shooter speed", shooterL.getVelocity().getValueAsDouble());
    ...
    SmartDashboard.putNumber(
        "Shooter/right integral value",
        shooterR.getClosedLoopIntegratedOutput().getValueAsDouble());
