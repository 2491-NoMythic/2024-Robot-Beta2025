...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  public void drive(ChassisSpeeds chassisSpeeds) {
    ...
    SwerveModuleState[] desiredStates =
        kinematics.toSwerveModuleStates(
            ChassisSpeeds.discretize(chassisSpeeds, 0.02)); // TODO see if this works as expected
   ...
   * DISCONTINUED
   *
   * <p>use calculateSpeakerAngleMoving() instead
   *
  ...
  public double calculateSpeakerAngle() {
    ...
    // RobotState.getInstance().ShooterInRange =
    // speakerDist<Field.MAX_SHOOTING_DISTANCE;
  ...
  public double calculateSpeakerAngleMoving() {
    ...
    // SmartDashboard.putString("offset speaker location", new
    // Translation2d(offsetSpeakerX,
    // offsetSpeakerY).toString());
    // getting desired robot angle
  ...
  public double calculateSpeakerAngleMovingWithSuppliers(
      DoubleSupplier redYSup,
      DoubleSupplier blueYSup,
      DoubleSupplier redRobotXSup,
      DoubleSupplier blueRobotXSup) {
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber(
        "CALLIBRATION/Supplied calculated robot angle",
        calculateSpeakerAngleMovingWithSuppliers(
            () -> SmartDashboard.getNumber("CALLIBRATION/redY", 0),
            () -> SmartDashboard.getNumber("CALLIBRATION/blueY", 0),
            () -> SmartDashboard.getNumber("CALLIBRATION/redRobotX", 0),
            () -> SmartDashboard.getNumber("CALLIBRATION/blueRobotX", 0)));
    for (int i = 0; i < 4; i++) {
      motorLoggers[i].log(modules[i].getDriveMotor());
    }
    ...
    // Logger.recordOutput("Vision/targetposes/NotePoses/FieldSpace",
    // robotToFieldCoordinates(LimelightHelpers.getTargetPose3d_RobotSpace(OBJ_DETECTION_LIMELIGHT_NAME)));
    // liamsEstimates();
  ...
  private Pose3d robotToFieldCoordinates(Pose3d robotCoordinates) {
    ...
    return new Pose3d(
        robotCoordinatesX + odometrPoseX,
        robotCoordinatesY + odometrPosey,
        robotCoordinatesZ,
        robotCoordinatesAngle);
  }
}
