...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    AmpAngleSup =
        () ->
            driverController.getPOV() == 90
                || driverController.getPOV() == 45
                || driverController.getPOV() == 135;
    ;
    ...
    SubwooferAngleSup =
        () -> driverController.getCrossButton() || operatorController.getCrossButton();
    StageAngleSup =
        () -> operatorController.getTriangleButton() || driverController.getTriangleButton();
    ;
    FarStageAngleSup =
        () -> operatorController.getSquareButton() || driverController.getSquareButton();
    OppositeStageShotSup =
        () -> operatorController.getCircleButton() || driverController.getCircleButton();
    ...
    if (intakeExists) {
      intakeInst(); /* Must happen before indexInit */
    }
    if (shooterExists) {
      shooterInst();
    }
    ...
    if (intakeExists && shooterExists && indexerExists && angleShooterExists) {
      indexCommandInst();
    }
    configureDriveTrain();
    configureBindings();
  ...
  private void angleShooterInst() {
    ...
    defaultShooterAngleCommand =
        new AimShooter(
            angleShooterSubsystem,
            AmpAngleSup,
            HumanPlaySup,
            SubwooferAngleSup,
            StageAngleSup,
  ...
  private void indexInit() {
    indexer =
        new IndexerSubsystem(intakeExists ? RobotState.getInstance()::isNoteSeen : () -> false);
  }

  private void indexCommandInst() {
    defaulNoteHandlingCommand =
        new IndexCommand(
            indexer,
            ShootIfReadySup,
            AimWhileMovingSup,
  ...
  private void configureBindings() {
    ...
    Command setGyroTo180 =
        new InstantCommand(() -> driveTrain.zeroGyroscope(180)) {
    ...
    // new Trigger(driverController::getCrossButton).onTrue(new
    // autoAimParallel(driveTrain/*,
    // shooter*/));
    new Trigger(ZeroGyroSup).onTrue(new InstantCommand(driveTrain::zeroGyroscope));
    // new Trigger(driverController::getCircleButton).whileTrue(new
    // GoToAmp(driveTrain)); unused
    // becuase we dont pickup from amp with a path anymore
    new Trigger(AimWhileMovingSup)
        .whileTrue(
            new AimRobotMoving(
                driveTrain,
                () -> modifyAxis(-driverController.getRawAxis(Z_AXIS), DEADBAND_NORMAL),
                ...
                falseSup));
    // new
    // Trigger(()->driverController.getL3Button()&&driverController.getR3Button()).onTrue(
    // new SequentialCommandGroup(
    // new InstantCommand(()->angleShooterSubsystem.setDesiredShooterAngle(22.5),
    // angleShooterSubsystem),
    // new ParallelRaceGroup(
    // new AimRobotMoving(driveTrain, zeroSup, zeroSup, zeroSup, ()->true, falseSup,
    // falseSup,
    // falseSup, falseSup, falseSup, ()->true),
    ...
    if (Preferences.getBoolean("Detector Limelight", false)) {
      ...
      autoPickup =
          new ParallelRaceGroup(
                  new SequentialCommandGroup(
                      new InstantCommand(
                          () -> angleShooterSubsystem.setDesiredShooterAngle(30),
                          angleShooterSubsystem),
                      new GroundIntake(intake, indexer, driveTrain, angleShooterSubsystem),
    ...
    new Trigger(ForceVisionSup)
        .onTrue(new InstantCommand(() -> SmartDashboard.putBoolean("force use limelight", true)))
        .onFalse(new InstantCommand(() -> SmartDashboard.putBoolean("force use limelight", false)));
    SmartDashboard.putData(
        "force update limelight position",
        new InstantCommand(() -> driveTrain.forceUpdateOdometryWithVision(), driveTrain));
    if (angleShooterExists) {
      new Trigger(ShooterUpManualSup)
          .whileTrue(
              new AngleShooter(
                  angleShooterSubsystem, () -> ShooterConstants.PRAC_MAXIMUM_SHOOTER_ANGLE));
      SmartDashboard.putData(
          "Manual Angle Shooter Up",
          new AngleShooter(
              angleShooterSubsystem, () -> ShooterConstants.PRAC_MAXIMUM_SHOOTER_ANGLE));
    }
    ...
    if (intakeExists && indexerExists) {
      new Trigger(GroundIntakeSup)
          .whileTrue(new GroundIntake(intake, indexer, driveTrain, angleShooterSubsystem));
    }
    if (intakeExists && indexerExists) {
      new Trigger(() -> RobotState.getInstance().isNoteSeen())
          .and(() -> !RobotState.getInstance().IsNoteHeld)
          .and(() -> DriverStation.isTeleop())
          .and(() -> !AimWhileMovingSup.getAsBoolean())
          .onTrue(
              new IndexerNoteAlign(indexer, intake)
     ...
     * Touchpad: manually turn on Intake (hold) [only works if intake code doesn't
     * exist in IndexCommand]
      ...
      SmartDashboard.putData(
          ...
          new SequentialCommandGroup(
              new InstantCommand(
                  () ->
                      intake.intakeYes(
                          IntakeConstants.INTAKE_SPEED, IntakeConstants.INTAKE_SIDE_SPEED),
                  intake)
    ...
    if (shooterExists) {
      SmartDashboard.putData(
          "shooter on speaker",
          new InstantCommand(() -> shooter.shootRPS(ShooterConstants.LONG_SHOOTING_RPS), shooter));
      SmartDashboard.putData(
          "shooter on amp",
          new InstantCommand(() -> shooter.shootRPS(ShooterConstants.PRAC_AMP_RPS), shooter));
      ...
      SmartDashboard.putData(
          "run shooter",
          new OverrideCommand(shooter) {
            @Override
            public void execute() {
              shooter.shootRPS(
                  SmartDashboard.getNumber(
                      "run shooter speed", ShooterConstants.LONG_SHOOTING_RPS));
            }
    ...
    if (angleShooterExists) {
      double testAngle = 45;
      SmartDashboard.putData(
          "go to angle", new AngleShooter(angleShooterSubsystem, () -> testAngle));
      SmartDashboard.putData(
          "run indexer down slow",
          new InstantCommand(
              () -> angleShooterSubsystem.pitchShooter(0.02), angleShooterSubsystem));
      SmartDashboard.putData(
          "run indexer up slow",
          new InstantCommand(
              () -> angleShooterSubsystem.pitchShooter(-0.02), angleShooterSubsystem));
      SmartDashboard.putData(
          "stop pitch",
          new InstantCommand(() -> angleShooterSubsystem.pitchShooter(0), angleShooterSubsystem));
      SmartDashboard.putData(
          "set reference to 60",
          new InstantCommand(() -> angleShooterSubsystem.setDesiredShooterAngle(60)));
      SmartDashboard.putData(
          "set reference to 30",
          new InstantCommand(() -> angleShooterSubsystem.setDesiredShooterAngle(45)));
      SmartDashboard.putData(
          "set reference to 15",
          new InstantCommand(() -> angleShooterSubsystem.setDesiredShooterAngle(17)));
    }
    if (indexerExists) {
      SmartDashboard.putData(
          "indexer intake speed",
          new InstantCommand(() -> indexer.set(IndexerConstants.INDEXER_INTAKE_SPEED)));
      SmartDashboard.putData(
          "indexer shooting speed",
          new InstantCommand(() -> indexer.set(IndexerConstants.INDEXER_SHOOTING_POWER)));
  ...
  private Pose2d getAmpShotPose(Alliance currentAlliance) {
    if (currentAlliance == null) {
      return new Pose2d(5, 5, new Rotation2d(Math.toRadians(-90)));
    } else {
  ...
  private void registerNamedCommands() {
    ...
    NamedCommands.registerCommand(
        "stopDrivetrain", new InstantCommand(driveTrain::stop, driveTrain));
    NamedCommands.registerCommand(
        "hardStop", new InstantCommand(driveTrain::pointWheelsInward, driveTrain));
    if (intakeExists && indexerExists) {
      NamedCommands.registerCommand(
          "driveBackwardsToIntake",
          new ParallelRaceGroup(
              new SequentialCommandGroup(
    ...
    if (shooterExists) {
      NamedCommands.registerCommand(
          "shooterOn", new InstantCommand(() -> shooter.shootRPS(LONG_SHOOTING_RPS), shooter));
      SmartDashboard.putData(
          "shooterOn", new InstantCommand(() -> shooter.shootRPS(LONG_SHOOTING_RPS), shooter));
    }
    if (intakeExists && indexerExists && angleShooterExists) {
      ...
      NamedCommands.registerCommand(
          ...
          new SequentialCommandGroup(
              new InstantCommand(
                  () -> angleShooterSubsystem.setDesiredShooterAngle(22.5), angleShooterSubsystem),
              new ParallelRaceGroup(
                  new AimRobotMoving(
                      driveTrain,
                      zeroSup,
      ...
      NamedCommands.registerCommand(
          "midAmpSideShooterAngle",
          new OverrideCommand(angleShooterSubsystem) {
            public void execute() {
    ...
    if (indexerExists) {
      // NamedCommands.registerCommand("feedShooter", new
      // InstantCommand(()->indexer.set(IndexerConstants.INDEXER_SHOOTING_SPEED),
      // indexer));
      // NamedCommands.registerCommand("stopFeedingShooter", new
      // InstantCommand(indexer::off,
      // indexer));
    }
    if (intakeExists) {
      NamedCommands.registerCommand(
      ...
      if (sideWheelsExists) {
        NamedCommands.registerCommand(
            "intakeSideWheels", new InstantCommand(() -> intake.intakeSideWheels(1)));
      }
      NamedCommands.registerCommand(
    ...
    if (indexerExists && shooterExists && angleShooterExists) {
      ...
      // AimRobotMoving), and
      // shoot a note while aiming the shooter (with shootNote). As a race group, it
      // ends
      // when either command finishes. the AimRobotMoving command will never finish,
      // but the
      // shootNote finishes when shootTime is reached.
      NamedCommands.registerCommand(
          "autoShootNote",
          new ParallelRaceGroup(
              new AimRobotMoving(
              ...
              new SequentialCommandGroup(
                  new InstantCommand(
                      () ->
                          angleShooterSubsystem.setDesiredShooterAngle(
                              angleShooterSubsystem.calculateSpeakerAngle()),
                      angleShooterSubsystem),
                  new ShootNote(indexer, 1.5, 0.5, intake))));
      // NamedCommands.registerCommand("setFeedTrue", new
      // InstantCommand(()->SmartDashboard.putBoolean("feedMotor", true)));
      // NamedCommands.registerCommand("setFeedFalse", new
      // InstantCommand(()->SmartDashboard.putBoolean("feedMotor", false)));
    }
    if (angleShooterExists) {
      // the same command that we use during teleop, but all the buttons that would
      // aim the shooter
      // anywhere other than the speaker are set to false.
      NamedCommands.registerCommand(
          "autoAimAtSpeaker",
          new AimShooter(
              angleShooterSubsystem,
    ...
    if (indexerExists && intakeExists) {
      NamedCommands.registerCommand(
          "groundIntake", new AutoGroundIntake(indexer, intake, driveTrain));
      SmartDashboard.putData("groundIntake", new AutoGroundIntake(indexer, intake, driveTrain));
    }
    NamedCommands.registerCommand(
        "wait x seconds", new WaitCommand(Preferences.getDouble("wait # of seconds", 0)));
  }
  ...
  public void ampShotInit() {
    if (indexerExists && shooterExists && angleShooterExists) {
      ...
      SequentialCommandGroup scoreAmp =
          new SequentialCommandGroup(
              // new InstantCommand(()->shooter.shootSameRPS(ShooterConstants.AMP_RPS),
              // shooter),
              new InstantCommand(
                  () -> shooter.shootWithSupplier(() -> shooterAmpSpeed, true), shooter),
              new MoveMeters(driveTrain, 0.06, 0.3, 0, 0),
              // new WaitCommand(2),
              new WaitUntil(
                  () ->
                      (shooter.validShot()
                          && driveTrain.getChassisSpeeds().vxMetersPerSecond == 0)),
              new InstantCommand(
                  () -> indexer.magicRPS(indexerAmpSpeed), indexer), // 45 worked but a bit too high
              new WaitCommand(0.5),
              new InstantCommand(() -> RobotState.getInstance().setNoteHeld(false)));
      SmartDashboard.putNumber("Indexer Amp Speed", indexerAmpSpeed);
      /**
       * the following code producs a command that will first pathfind to a pose right in front of
       * the amplifier, then drive backwards into the amp, then run our shooters amp shot sequence
       * as it was at the 2024 State competition.
       */
      SequentialCommandGroup orbitAmpShot =
          new SequentialCommandGroup(
              new InstantCommand(
                  () -> shooter.setTargetVelocity(shooterAmpSpeed, shooterAmpSpeed, 50, 50),
                  shooter),
              new InstantCommand(
                  () -> angleShooterSubsystem.setDesiredShooterAngle(50), angleShooterSubsystem),
              // new
              // AutoBuilder().pathfindThenFollowPath(PathPlannerPath.fromPathFile("AmpShotSetup"),
              // DEFAUL_PATH_CONSTRAINTS),
              Commands.select(
                  Map.of(
                      Alliance.Red,
                      AutoBuilder.pathfindToPose(
                          getAmpShotPose(Alliance.Red), DEFAULT_PATH_CONSTRAINTS),
                      Alliance.Blue,
                      AutoBuilder.pathfindToPose(
                          getAmpShotPose(Alliance.Blue), DEFAULT_PATH_CONSTRAINTS)),
              ...
              new WaitUntil(
                  () ->
                      (Math.abs(shooter.getLSpeed() - shooterAmpSpeed) < 0.2)
                          && (Math.abs(shooter.getRSpeed() - shooterAmpSpeed) < 0.3)),
              new InstantCommand(
                  () ->
  ...
  public void teleopInit() {
    if (climberExists) {
      SequentialCommandGroup resetClimbers =
          new SequentialCommandGroup(
              new InstantCommand(
                  () -> {
                    climber.climberGoLeft(ClimberConstants.CLIMBER_SPEED_DOWN);
